...
namespace LoLSharp.Modules
{
  class NativeImport
  {
    ...
    public static UInt64 ReadUInt64(IntPtr hProcess, UInt64 addr) {
      byte[] buffer = new byte[8];
      int bytesRead = 0;
      ReadProcessMemory(hProcess, (IntPtr)addr, buffer, buffer.Length, out bytesRead);
      return BitConverter.ToUInt64(buffer, 0);
    }

    public static void Read(IntPtr hProcess, UInt64 addr, byte[] buffer, int size) {
      int bytesRead = 0;
      ReadProcessMemory(hProcess, (IntPtr)addr, buffer, size, out bytesRead);
    }

    public static UInt64 ReadUInt64FromBuffer(byte[] buffer, int position) {
